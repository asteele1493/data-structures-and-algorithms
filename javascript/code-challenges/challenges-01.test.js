...
const addOne = (arr) => {
  const localArr = [];
  //use for each to loop over the input array.
...
const addExclamation = (arr) => {
  ...
  const newArr = [];
  //for each string in the array, I will push the string with four exclamation points into my new array.
  arr.forEach(stringz => newArr.push(stringz + '!'));
  return newArr;
};
...
const allUpperCase = (arr) => {
  //initializing new array
  const anotherArr = [];
  //the arrow function will return and push each string to uppercase to my anotherArr.
  arr.forEach(str => anotherArr.push(str.toUpperCase()));
  return anotherArr;
};
...
const speaker = (words, callback) => {
  ...
  const yetAnotherArr = [];
  //each string(word) should be modified by the callback
  words.forEach(word => yetAnotherArr.push(callback(word)));

  //return new array of strings
  return yetAnotherArr;
};

// Found this link to be a helpful resource in helping us solve this challenge: https://www.w3schools.com/js/js_callback.asp
// Pair programmed with Manuch
...
//Pair programmed with Manuch-- got to our for loop and had trouble getting past where to go next. Camilla helped get us unstuck. Definitely overthought the process.
/* ------------------------------------------------------------------------------------------------
